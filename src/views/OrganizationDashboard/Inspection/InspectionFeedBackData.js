  ...
  render() {
    ...
    for(const [i, feedback] of this.props.data.entries()){
      ...
      let feedbackInfo = {
        ...
        action: <p><Link to={`/organization/manage-inspection/inspection/${feedback.inspectionId}/${feedback._id}`} title="View Feddback" className="btn-view" disabled={this.state.buttonProcessing} ><i className="fa fa-eye"></i> </Link>
          <button className="btn-delete" title="Delete Feedback" disabled={this.state.buttonProcessing}><i className="fa fa-trash"></i></button></p>,       
      }      
